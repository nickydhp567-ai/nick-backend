...
router.post('/ussd', async (req, res) => {
  ...
  const input = text || '';
  const ph = (phoneNumber||'').replace('+233','0').replace(/^233/,'0');
  res.set('Content-Type','text/plain');
  const {User,Account,Savings} = require('./models');
  const {verifyPin} = require('./services/authService');
  const {round2} = require('./utils/domain');
  function con(m){res.send('CON '+m);}
  function end(m){res.send('END '+m);}
  try {
    if(input==='0') return end('Thank you for banking with NICK Bank!');
    if(input==='1') return con('My Bank
...
Thank you!');
    ...
    if(input==='2') return con('Back Pocket

1. View Savings
0. Back');
    if(input==='2*1') return con('Enter your PIN:');
...
NICK Bank');
  ...
  } catch(e){ return end('Error: '+e.message); }
});
