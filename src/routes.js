...
router.post('/ussd', async (req, res) => {
  try {
    ...
    if (text.length === 0) return con('Welcome to NICK Bank PLC\n\n1. My Bank\n2. Back Pocket\n3. Balance\n0. Exit');
...
router.get('/admin/users', async (req, res) => {
  try {
    ...
    const users = await User.findAll({
      attributes: ['id', 'full_name', 'phone', 'email', 'roundup_preference', 'createdAt'],
      include: [
        { model: Account, attributes: ['type', 'balance', 'account_number'] },
        { model: Savings, attributes: ['total', 'roundup_total', 'standing_total'] },
      ],
